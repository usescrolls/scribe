...
describe('useWorkspaces', () => {
  ...
  describe('switchWorkspace', () => {
    it('calls SetActiveWorkspace and refreshes', async () => {
      ...
      mockAppService.GetWorkspaces
        .mockResolvedValueOnce(mockWorkspaces) // initial fetch
        .mockResolvedValueOnce(workspacesAfterSwitch) // after switch

      const { switchWorkspace, activeWorkspace, fetchWorkspaces } = useWorkspaces()
      await fetchWorkspaces()
    ...
    it('returns false and sets error on failure', async () => {
      mockAppService.SetActiveWorkspace.mockRejectedValue(new Error('Workspace not found'))
  ...
  describe('createWorkspace', () => {
    it('creates workspace and refreshes list', async () => {
      ...
      const result = await createWorkspace('my-workspace', 'My custom workspace')

      expect(mockAppService.CreateWorkspace).toHaveBeenCalledWith('my-workspace', 'My custom workspace')
      expect(result).toBe(true)
    })
    ...
    it('returns false on error', async () => {
      mockAppService.CreateWorkspace.mockRejectedValue(new Error('Name already exists'))
  ...
  describe('deleteWorkspace', () => {
    ...
    it('returns false and sets error on failure', async () => {
      mockAppService.DeleteWorkspace.mockRejectedValue(new Error('Cannot delete default workspace'))
